  ...
  <div class="flex-align-start order-detail h-full">
    <div class="left">
      <n-descriptions label-placement="left" bordered :column="2">
        ...
        <n-descriptions-item label="订单业务类型" :span="2">{{
          order?.orderBusinessType
        }}</n-descriptions-item>
        <n-descriptions-item label="订单类型" :span="2">{{ order?.orderType }}</n-descriptions-item>
        <n-descriptions-item label="下单客户电话" :span="2">{{
          order?.customerPhone
        }}</n-descriptions-item>
        <n-descriptions-item label="接单司机姓名" :span="2">{{
          order?.driverFullName
        }}</n-descriptions-item>
        <n-descriptions-item label="接单司机工号" :span="2">{{
          order?.driverNo
        }}</n-descriptions-item>
        <n-descriptions-item label="接单司机电话" :span="2">{{
          order?.driverPhone
        }}</n-descriptions-item>
        <n-descriptions-item label="接单车型" :span="2">{{
          order?.plateNumber
        }}</n-descriptions-item>
        <n-descriptions-item label="订单状态" :span="2">{{
          order?.orderState
        }}</n-descriptions-item>
      </n-descriptions>
      <div class="m-10px flex-align-start mt-30px">
        ...
        >
          ...
          <n-form-item path="loginPassword" label="管理员密码">
            <n-input
              style="width: 200px"
...
import { DispatchQueryForm } from "@/views/operateOrder/servingOrder/type";
import {
  artificialDistributeOrder,
  findByDriverNoHeaderUnlock,
  getOrderDetail,
} from "@/api/operateOrder/operateOrder";
import { useRoute, useRouter } from "vue-router";
import md5 from "blueimp-md5";
import { getDetailViaLoginer } from "@/api/system/system";
import { refreshDriverPosition } from "@/api/common/common";
...
export default defineComponent({
  ...
  setup() {
    ...
    const queryForm = ref<DispatchQueryForm>({
      orderId: null,
    ...
    const getData = async (orderId: string) => {
      ...
      try {
        ...
        if (res.success && res.data != null) {
          ...
          initMap(
            res.data.orderBeginAddressLongitude * 1e-6,
            res.data.orderBeginAddressLatitude * 1e-6
          );
        } else {
    ...
    const execute = async () => {
      ...
      let data = await refreshDriverPosition({
        operationCompanyId: order.value.operationCompanyId,
      });
      if (data.result) {
          ...
          if (driverPosition) {
            ...
            if (!driverMarkerMap.get(driverPosition.operationCompanyDriverId)) {
              let marker = addMarker(
                ...
                driverPosition.driverServiceState == "DSS0001"
                  ? freeIconImg
                  : driverPosition.driverServiceState == "DSS0003"
                  ? restIconImg
                  : driverPosition.driverServiceState == "DSS0002"
                  ? busyIconImg
                  : offDutyIconImg
              );
          ...
          try {
            ...
            if (data.success) {
              content =
                ...
                "<br/>" +
                "司机姓名 : " +
                data.data.driverFullName +
                "<br/>" +
                "司机电话 : " +
                data.data.driverPhone +
                "<br/>" +
                "车牌号 : " +
