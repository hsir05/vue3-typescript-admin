<template>
  <BasicDrawer v-model:show="isDrawer" title="兑换码详情" @on-close-after="onCloseAfter">
    ...
    >
      ...
      <n-descriptions-item label="代金券面值(元)">{{
        item?.couponDenomination
      }}</n-descriptions-item>
      <n-descriptions-item label="代金券数量">{{ item?.couponCount }}</n-descriptions-item>
      <n-descriptions-item label="代金券有效天数">{{
        item?.couponEffectiveDays
      }}</n-descriptions-item>
      <n-descriptions-item label="使用限制金额(元)"
        >{{ item?.customerCouponConsumeRule?.couponLimitedAmount }}
      </n-descriptions-item>
      <n-descriptions-item label="可使用的城市"
        >{{ item?.customerCouponConsumeRule?.couponUsedCityNames }}
      </n-descriptions-item>
      <n-descriptions-item label="可使用的车型" :span="2"
        >{{ item?.customerCouponConsumeRule?.couponUsedVehicleTypeNames }}
      </n-descriptions-item>
      <n-descriptions-item label="可使用的时间"
        >{{ item?.customerCouponConsumeRule?.couponUsedDayTimeSectionDesc }}
      </n-descriptions-item>
      <n-descriptions-item label="可使用的星期"
        >{{ item?.customerCouponConsumeRule?.couponUsedWeekSectionDesc }}
      </n-descriptions-item>
      <n-descriptions-item label="可使用的订单类型" :span="2"
        >{{ item?.customerCouponConsumeRule?.couponUsedOrderTypeNames }}
      </n-descriptions-item>
    </n-descriptions>
  </BasicDrawer>
</template>
