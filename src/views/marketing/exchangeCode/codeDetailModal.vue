<template>
  ...
  >
    <n-descriptions label-placement="left" bordered :column="2" size="large">
      ...
      <n-descriptions-item label="开始时间" :span="2">{{
        dayjs(data?.beginTime).format("YYYY-MM-DD HH:mm:ss")
      }}</n-descriptions-item>
      <n-descriptions-item label="结束时间" :span="2">{{
        data?.endTime != null ? dayjs(data.endTime).format("YYYY-MM-DD HH:mm:ss") : "暂无"
      }}</n-descriptions-item>
