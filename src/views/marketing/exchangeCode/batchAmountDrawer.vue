  ...
  >
    ...
    >
      <n-form-item label="开始时间" path="beginTimeGe">
        <n-date-picker
          v-model:value="form.beginTimeGe"
          style="width: 210px"
          type="date"
          clearable
        />
      ...
      <n-form-item label="结束时间" path="beginTimeLe">
        <n-date-picker
          v-model:value="form.beginTimeLe"
          type="date"
          style="width: 210px"
          clearable
        />
...
export default defineComponent({
  ...
  components: { AmountDrawer, CodeDetailModal, BasicTable },
  emits: ["on-save-after"],
  setup() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render(record: DataImportTaskITemInter) {
          return h("span", dayjs(record.beginTime).format("YYYY-MM-DD HH:mm:ss"));
        },
      },
      {
        title: "总条数",
      ...
      },
      {
        ...
        align: "center",
        render(record: DataImportTaskITemInter) {
          return h("span", record.endTime === null ? "生成中" : "已完成");
        },
      },
      {
        title: "操作人",
    ...
    function handleRefresh(record: Recordable) {
      const index = data.value?.findIndex(
        (item: { dataImportTaskId: string }) => item.dataImportTaskId === record.dataImportTaskId
      );
